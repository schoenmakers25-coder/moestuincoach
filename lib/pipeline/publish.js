...
import { SITE_AUTHOR } from '../articles.js'
...
// Prefix van het commit-bericht dat publishArticle() gebruikt. De
// idempotentie-check herkent hieraan een échte wekelijkse publicatie, zodat
// een gewone content-edit (typefix, opmaakfix) de wekelijkse run niet blokkeert.
const WEEKLY_COMMIT_PREFIX = 'Wekelijks artikel:'
...
export async function publishedSince(sinceIso) {
  ...
  const url = `${GITHUB_API}/repos/${owner}/${name}/commits?path=${encodeURIComponent(CONTENT_PATH)}&sha=${encodeURIComponent(branch)}&since=${encodeURIComponent(sinceIso)}&per_page=30`
  ...
  // Alleen echte wekelijkse publicaties tellen, niet elke content-edit.
  return commits.some(c => (c?.commit?.message || '').startsWith(WEEKLY_COMMIT_PREFIX))
}
...
export async function publishArticle({ article, image, dateIso }) {
  ...
  const resp = await fetch(url, {
    ...
    body: JSON.stringify({
      message: `${WEEKLY_COMMIT_PREFIX} ${article.title}${status === 'draft' ? ' (concept)' : ''}`,
      content,
      branch,
    }),
  })
