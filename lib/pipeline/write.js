...
// Ruim boven wat een artikel van 900 tot 1500 woorden plus faq nodig heeft,
// zodat het JSON niet halverwege afkapt.
...
export async function writeArticle({ topic, season, existingArticles }) {
  ...
  const prompt = `Je bent Sanne de Groot, de moestuinredacteur van moestuin.nl. Schrijf een compleet,
...
- GEEN LIGGENDE STREEPJES: gebruik NERGENS een em-dash (Unicode U+2014) of en-dash
  (U+2013), in geen
  enkel veld (title, metaDescription, excerpt, koppen, bodyMarkdown, faq). Vervang ze
  door gewone interpunctie: een komma, punt of dubbele punt. Schrijf getalreeksen
  voluit met "tot" (bijvoorbeeld "3 tot 5 cm" of "20 tot 30°C"), nooit met een streepje.
...
Lever alles als het gevraagde JSON-object.`
  ...
  // Twee pogingen: een lang artikel liep bij 8000 tokens tegen het plafond,
  // waardoor het JSON halverwege afkapte en de hele wekelijkse run afbrak
  // (17-08-2026, fase "schrijven"). Ruimer plafond, stop_reason expliciet
  // afvangen en bij afkapping een tweede, strakkere poging.
  let article = null
  let lastError = null
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    const retryHint = attempt === 1
      ? ''
      : '\n\nLET OP: de vorige poging werd afgekapt omdat het antwoord te lang werd. ' +
        'Houd het deze keer strak: 900 tot 1100 woorden en maximaal 3 faq-vragen.'
    ...
    if (resp.stop_reason === 'max_tokens') {
      lastError = new Error(
        `Schrijfstap werd afgekapt op de tokenlimiet (${WRITER_MAX_TOKENS}), poging ${attempt} van 2.`
      )
      console.warn(`[write] ${lastError.message}`)
      continue
    }
    ...
    try {
      ...
      lastError = new Error(
        `Schrijfstap gaf geen parse-baar JSON terug (poging ${attempt} van 2, ` +
        `stop_reason: ${resp.stop_reason}): ${jsonText.slice(0, 300)}`
      )
      console.warn(`[write] ${lastError.message}`)
    }
  }

  if (!article) throw lastError
